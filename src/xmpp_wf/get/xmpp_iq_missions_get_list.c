...
static void get_mission_list ( const char *msg )
{
...
static void xmpp_iq_missions_get_list_cb(const char *msg, void *args)
{
	...
	mission_list_free ( );
	mission_list_init ( );
	get_mission_list ( msg );
	LOGPRINT ( "%-16s "BOLD"%d MISSIONS\n", "RETRIEVED", (int) session.missions->length );
}
...
void xmpp_iq_missions_get_list( )
{
    ...
    idh_register(&id, 0, xmpp_iq_missions_get_list_cb, NULL);
